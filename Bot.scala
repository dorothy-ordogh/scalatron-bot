...
object ControlFunction
{
    def forMaster(bot: Bot) {
        ...
        val numSpawn = bot.inputAsIntOrElse("spawn", 0)
        ...
        if(bot.energy > 1500 && numSpawn <= 60) {  // release foraging bots?
            bot.spawn(direction, "mood" -> "Foraging")
            bot.set("spawn" -> (numSpawn + 1))
        }
    ...
    def forSlave(bot: MiniBot) {
        bot.inputOrElse("mood", "Lurking") match {
            case "Foraging" => reactAsForager(bot)
    ...
    def reactAsForager(bot: MiniBot) {
        val (directionValue, nearestEnemyMaster, nearestEnemySlave) = analyzeViewAsSlave(bot.view)
        val lastDirection = bot.inputAsIntOrElse("lastDirection", 0)
        // determine movement direction
        directionValue(lastDirection) += 10 // try to break ties by favoring the last direction
        val bestDirection45 = directionValue.zipWithIndex.maxBy(_._1)._2
    ...
    /** Analyze the mini-bot's view, building a map of attractiveness for the 
     * 45-degree directions and recording other relevant data, such as the nearest
    ...
    def analyzeViewAsSlave(view: View) = {
        ...
        for(i <- 0 until cellCount) {
            ...
            if(cellRelPos.isNonZero) {
                val stepDistance = cellRelPos.stepCount
                val value: Double = cells(i) match {
                    case 'm' => // another master: dangerous!
                        nearestEnemyMaster = Some(cellRelPos)
                        if(stepDistance < 2) -1000 else 0
